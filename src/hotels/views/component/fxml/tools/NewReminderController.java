...
public class NewReminderController implements Initializable {
    ...
    public NewReminderController(Hotels app) {
        this.app = app;
    ...
    private void newReminder(){
        ...
        Runnable task = new Runnable() {
            ...
            public void run() {
                    ...
                    response = nav.createNewReminder(param);
                    System.out.println("Printing Created Reminder : " +response);
                    if(response != null){
                        Platform.runLater(new Runnable(){
                            @Override
                            public void run() {
                                Util.notify(State.NOTIFY_SUCCESS, "A New Reminder is Set", Pos.CENTER);
                            }
                        });
                    }else{
                        ...
                        Platform.runLater(new Runnable(){
                            @Override
                            public void run() {
                                Util.notify(State.NOTIFY_ERROR, "New Reminder Setting Failed", Pos.CENTER);
                            }
                        });
                    }   
