 ...
 * @author NOVA
 */
public class NewPayOutController implements Initializable {
    ...
    public NewPayOutController(Hotels app) {
        this.app = app;
    ...
    @FXML private void newPayout(){
        ...
        if(!isEditMode()){
            ...
            Runnable task = new Runnable() {
                ...
                public void run() {
                    try {
                        response = nav.createPayout(param);
                        if(response != null && response.getInt("status") == 1){
                            Platform.runLater(new Runnable(){
                                @Override
                                public void run() {
                                    Util.notify(State.NOTIFY_SUCCESS, "New Pay Out Created and Saved", Pos.CENTER);
                                }
                            });
                        }else{
                            ...
                            Platform.runLater(new Runnable(){
                                @Override
                                public void run() {
                                    Util.notify(State.NOTIFY_ERROR, "Pay Out Failed to Create", Pos.CENTER);
                                }
                            });
                        }
                    ...
                    response = nav.editHotelService(param);
                        if(response.getInt("status") == 1){
                            Platform.runLater(new Runnable(){
                                @Override
                                public void run() {
                                    Util.notify(State.NOTIFY_SUCCESS, "Pay Out Data Updated", Pos.CENTER);
                                }
                            });
                        }else{
                            ...
                            Platform.runLater(new Runnable(){
                                @Override
                                public void run() {
                                    Util.notify(State.NOTIFY_ERROR, "Pay Out Data Failed to Update", Pos.CENTER);
                                }
                            });
                        }
                    } catch (JSONException ex) {
